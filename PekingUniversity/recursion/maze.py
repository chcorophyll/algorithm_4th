        ...
        for y in range(self.rows_in_maze):
            for x in range(self.columns_in_maze):
                if self.maze_list[y][x] == OBSTACLE:
                    self.draw_center_box(x+self.x_translate, -y+self.y_translate, "orange")
        self.t.color("black")
...
def search_from(maze, start_row, start_column):
    ...
    if maze[start_row][start_column] == TRIED or \
        ...
        return False
    if maze.is_exist(start_row, start_column):
        maze.update_position(start_nrw, start_column, PART_OF_PATH)
        return True
